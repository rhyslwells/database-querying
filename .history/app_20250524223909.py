...
# Initialize session state for DB connection and schema status
...
# Step 1: Upload or provide schema
...
if uploaded_db:
    ...
    os.rename(uploaded_db.name, 'uploaded_database.db')  # Save uploaded DB file locally
...
if st.session_state.conn:
    ...
    if st.button("Download current database"):
        db_path = 'my_database.db' if st.session_state.schema_executed else None
    ...
    if st.button("Reset Application"):
        ...
        if os.path.exists("uploaded_database.db"):
