...
def app():
    ...
    if not st.session_state.schema_executed:
        ...
        if schema_input_mode == "Upload SQL file":
            ...
            st.session_state.schema_sql = st.text_area("Enter your SQL schema here:", st.session_state.schema_sql, height=200)

        # Display preview always when schema_sql is available
        if st.session_state.schema_sql:
            st.text_area("Schema Preview", st.session_state.schema_sql, height=150, disabled=True)

        # Execute schema if not done yet
