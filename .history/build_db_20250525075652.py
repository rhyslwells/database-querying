...
def app():
    ...
    if "schema_sql" not in st.session_state:
        st.session_state.schema_sql = ""  # Initialize with empty string

    # Step 1: Provide schema
    # Step 1: Provide schema or show preview if already executed
    ...
    if not st.session_state.schema_executed:
        ...
        # Schema already executed → show preview (disabled)
        if st.session_state.schema_sql:
