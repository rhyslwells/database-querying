...
def app():
    ...
    if not st.session_state.schema_executed:
        ...
        if st.session_state.schema_sql and not st.session_state.schema_executed:
            ...
            # Create new connection with thread check disabled
            db_filename = get_timestamped_db_name()
            conn = create_connection(db_filename)  # your create_connection sets check_same_thread=False
    ...
    # Step 5: ER Diagram visualization
    if st.session_state.conn:
