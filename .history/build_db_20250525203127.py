...
def app():
    ...
    if "db_bytes" not in st.session_state:
        ...
        st.header("Try a Sample Database")

    if st.button("Load Example Database from `sample_data/longlist.db`"):
        ...
        if os.path.exists(example_path):
            ...
            # Reset other schema-related state
            st.session_state.schema_executed = True
            st.session_state.schema_sql = "-- Loaded from example DB: sample_data/longlist.db"

            st.success("Example database loaded into memory.")
        else:
            st.error("Example database not found at `sample_data/longlist.db`.")
