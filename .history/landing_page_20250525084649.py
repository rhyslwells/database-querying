...
def app():
    ...
    This application provides two main workflows to interact with SQLite databases entirely within your browser:

    1. **Upload Existing DB**  
       Upload a `.db` SQLite database file and run SQL queries on it instantly, without any setup.

    2. **Build DB from CSV**  
       Create a new SQLite database by providing a schema and uploading CSV files to populate tables.
