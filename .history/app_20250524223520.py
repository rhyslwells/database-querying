...
# Session state for DB connection and schema execution
...
if uploaded_db:
    ...
    if schema_input_mode == "Upload SQL file":
        schema_sql = upload_sql_schema()  # from file_uploads.py
        if schema_sql:
            st.text_area("Schema Preview (Uploaded)", schema_sql, height=150, key="uploaded_schema_preview", disabled=True)
    elif schema_input_mode == "Type SQL schema":
        schema_sql = st.text_area("Enter your SQL schema here:", height=200, key="typed_schema")
